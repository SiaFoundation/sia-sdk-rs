                ...
                Some(res) = shard_tasks.join_next() => {
                    ...
                    match result {
                        Ok(data) => {
                            ...
                            debug!("slab {} shard {} download from {} complete after {elapsed:?} (attempt {}/{MAX_SECTOR_ATTEMPTS}", self.slab_index, task.shard_index, task.sector.host_key, task.attempts)
                        },
                        Err(e) => {
                            let mut task = task;
                            task.attempts += 1;
                            warn!(
                                "slab {} shard {} download from {} failed after {elapsed:?} (attempt {}/{MAX_SECTOR_ATTEMPTS}): {e}",
