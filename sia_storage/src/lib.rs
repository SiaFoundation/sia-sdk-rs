...
mod test {
    ...
    async fn test_download_write_to_path() {
        ...
        // large enough that the chunk size ramps and the last chunk is partial,
        // so the write covers more than one buffered chunk.
        let mut input = BytesMut::zeroed(300_000);
        random_bytes(&mut input);
        let input = input.freeze();
