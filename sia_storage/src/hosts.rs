...
use crate::hosts::metrics::{HostMetric, HostScore, RPCLatency, Transfer};
...
pub(crate) struct Hosts<T: Transport> {
    ...
    write_latency: Arc<RPCLatency>,
    read_latency: Arc<RPCLatency>,
}
...
impl<T: Transport> Hosts<T> {
    pub fn new(transport: T) -> Self {
        Self {
            ...
            write_latency: Arc::new(RPCLatency::new(Transfer::new(
                SECTOR_SIZE as u32,
                Duration::from_millis(50),
            ))),
            read_latency: Arc::new(RPCLatency::new(Transfer::new(
                1 << 20,
                Duration::from_millis(50),
            ))),
        }
    ...
    /// write latency window. Zero-size or zero-duration transfers are
    ...
    pub fn record_write_sample(&self, host_key: PublicKey, size: u32, elapsed: Duration) {
        ...
        self.write_latency.record(transfer);
    }
    ...
    pub fn record_read_sample(&self, host_key: PublicKey, size: u32, elapsed: Duration) {
        ...
        self.read_latency.record(transfer);
    }

    /// Returns the elapsed time after which an in-flight upload of `bytes`
    /// is slow enough to race.
    pub fn write_race_timeout(&self, bytes: u32) -> Duration {
    ...
    /// Download equivalent of [`Self::write_race_timeout`], based on the
    /// p95 of recent successful reads.
    pub fn read_race_timeout(&self, bytes: u32) -> Duration {
