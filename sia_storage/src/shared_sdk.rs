...
impl SharedSdk {
    ...
    pub async fn connect<U: IntoUrl>(indexer_url: U, seed: [u8; 32]) -> Result<Self, BuilderError> {
        ...
        Self::with_backends(api_client, Client::new(), PrivateKey::from_seed(&seed)).await
    }
    ...
    pub(crate) async fn with_backends(
        ...
        let warm_task = Self::spawn_warm_task(hosts.clone());
        Ok(Self {
            sharing_key,
            api_client,
            hosts,
    ...
    fn spawn_warm_task(hosts: Hosts) -> AbortOnDropHandle<()> {
        AbortOnDropHandle::new(maybe_spawn!(async move {
            loop {
                hosts.warm_connections(hosts.endpoints()).await;
